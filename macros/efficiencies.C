...
void efficiencies(TString suffix = ""){
  ...
  // Defining Samples
  std::vector<TString> signalSamples = {"TstarTstar_M-700.root", "TstarTstar_M-1000.root", "TstarTstar_M-1600.root", "TstarTstar_M-2000.root"};
  std::vector<TString> BGSamples = {"TTbar.root", "ST.root", "WJets.root"};
  //std::vector<TString> BGSamples = {};

  std::vector<TString> signal_labels = {"Tstar M-700", "Tstar M-1000","Tstar M-1600", "Tstar M-2000"};
  std::vector<TString> BG_labels = {"TTbar", "ST", "WJets"};

  // Defining Drawing options
  std::vector<int> colors_Signal = {1, 1, 1, 1};
  std::vector<int> line_Signal = {2, 3, 4, 5};
  std::vector<int> colors_BG = {810, 800, 600};
