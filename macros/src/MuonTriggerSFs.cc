...
int main(int argc, char* argv[]){
  ...
  h_pt_data.push_back(new TH1F("h_pt_muon_data","pt", pt_bins.size()-1, &pt_bins[0]));
  ...
  h_pt_mc.push_back(new TH1F("h_pt_muon_mc","pt", pt_bins.size()-1, &pt_bins[0]));
  ...
  TFile *f_data=new TFile(fdir+"uhh2.AnalysisModuleRunner.DATA.SingleElectron"+year_v+".root");
  ...
  */
  ...
  if(argc == 2){
    ...
    TFile * f_out = new TFile("files/MuonTriggerSF"+year+".root","RECREATE");;
...
void PlotEfficiency(TGraphAsymmErrors* h_data, TGraphAsymmErrors* h_mc, TString xaxis, TString histname){
  ...
  else if(weighttag == "weight_sfetaptUP") A->SaveAs("plots/"+histname+"_"+year+".pdf");
  else if(weighttag == "weight_sfetaptDOWN") A->SaveAs("plots/"+histname+"_"+year+".pdf");
  else A->SaveAs("plots/"+histname+"_"+year+".pdf");
  delete A;
  return;
}
...
void CompareHists(TH1F* h_all, TH1F* h_pass, TString xaxis, TString histname){
  ...
  gPad->RedrawAxis();
  if(weighttag == "weight") A->SaveAs("plots/"+histname+"_"+year+".pdf");
  delete A;
  return;
}
...
void DrawControl(TString path, VecH hists, TH1F* rData, TH1F* rMC, VecI colors, VecTS names, TString xaxis)
{
  TString unique_path = "plots/Control_"+path+".pdf";
