...
namespace {
  ...
  double chi2Function(ReconstructionTstarHypothesis hyp){
    ...
    double chi2_top_lep = pow((ttbar_hyp.tophad_topjet_ptr()->softdropmass() - (mtoplep_))/(sigmatoplep_) ,2);
    chi2 += chi2_top_lep;

    double sigma_deltaM_Tstar = 20.;
    double chi2_deltaM_Tstar = pow( (inv_mass(hyp.tstarlep_v4()) - inv_mass(hyp.tstarhad_v4()))/(sigma_deltaM_Tstar) ,2);
...
// ####################
// ####################
// ####################       Stuff below this is unused atm!
// ####################
// ####################


TstarTstarTopTagReconstruction::TstarTstarTopTagReconstruction(Context & ctx, const NeutrinoReconstructionMethod & neutrinofunction, const string & label, TopJetId tjetid, float minDR_tj_j):
...
bool TstarTstarTopTagReconstruction::process(uhh2::Event & event) {

  bool debug = false;

  if(debug){cout << "Hello World from TstarTstarTopTagReconstruction!" << endl;}
  ...
  for(const auto& tj : *event.topjets){
    ...
    if(debug){cout << "Starting to get ak4 jets not overlapping with gluons!" << endl;}
    ...
    for(const auto & jet : *event.jets)
      if(deltaR(tj, jet) > minDR_topjet_jet_){
	if(topjetCount == 0 && (deltaR(event.topjets->at(1), jet) > minDR_topjet_jet_) && (deltaR(event.topjets->at(2), jet) > minDR_topjet_jet_)){tlep_jets.push_back(&jet);}
	else if(topjetCount == 1 && (deltaR(event.topjets->at(0), jet) > minDR_topjet_jet_) && (deltaR(event.topjets->at(2), jet) > minDR_topjet_jet_)){tlep_jets.push_back(&jet);}
	else if(topjetCount > 1 && (deltaR(event.topjets->at(0), jet) > minDR_topjet_jet_) && (deltaR(event.topjets->at(1), jet) > minDR_topjet_jet_)){tlep_jets.push_back(&jet);}
      }
...
bool ttbarChi2Discriminator::process(uhh2::Event& event){
  ...
  // TODO check if this event is a ttagevent
  bool foundttagJet = false;
  for(uint i = 0; i < event.topjets->size(); i++){
    auto toptaggedjet = *bestCand.tophad_topjet_ptr();
    if(toptaggedjet.v4() == event.topjets->at(i).v4()){
      if(foundttagJet){cout << "ERROR, two identical ttaged jets found!" << endl; return false;}
      foundttagJet = true;
      event.set(h_ttag_jet_pos, i);
    }
  }
  if(!foundttagJet){cout << "ERROR: No ttaged jet in ttag event" << endl;}
...
bool TstarTstar_tgluon_tgluon_Reconstruction::process(uhh2::Event& event){
  ...
  if(event.get(h_flag_toptagevent) == 1){
    if(debug){cout << "Begin Reconstruction with TTag" << endl;}
    ...
    if(debug){cout << "Begin Reconstruction without TTag" << endl;}
    std::vector<Jet> used_jets_ = hyp_ttbar.tophad_jets();
    used_jets_.insert( used_jets_.end(), hyp_ttbar.toplep_jets().begin(), hyp_ttbar.toplep_jets().end() );
...
TstarTstar_tgluon_tgluon_Reconstruction2::TstarTstar_tgluon_tgluon_Reconstruction2(uhh2::Context& ctx){
  h_recohyp_ttbar_ = ctx.get_handle<ReconstructionHypothesis>("TTbarReconstruction_best");
  h_recohyp_tstartstar_best_ = ctx.get_handle<ReconstructionTstarHypothesis>("TstarTstar_tgtg_best");
  h_flag_toptagevent = ctx.get_handle<int>("flag_toptagevent");
  h_ttag_jet_pos = ctx.get_handle<int>("ttag_jet_pos");
...
bool TstarTstar_tgluon_tgluon_Reconstruction2::process(uhh2::Event& event){

  bool debug = false;

  if(debug){cout << "Hello World from Tstartstar_tgluon_tgluon_Reconstruction2! Yes, 2! Why? Its better than 1." << endl;}
  ...
  if(event.get(h_ttag_jet_pos) == 0){notused_topjets_.push_back(event.topjets->at(1)); notused_topjets_.push_back(event.topjets->at(2));}
  else if(event.get(h_ttag_jet_pos) == 1){notused_topjets_.push_back(event.topjets->at(0)); notused_topjets_.push_back(event.topjets->at(2));}
  else if(event.get(h_ttag_jet_pos) > 1){notused_topjets_.push_back(event.topjets->at(0)); notused_topjets_.push_back(event.topjets->at(1));}


  if(debug) cout<<"Number of not used in ttbar AK8 jets is "<<notused_topjets_.size()<<endl;
  if(notused_topjets_.size()<2) return false; // Require two AK8 Jets
...
// ####################
// ####################
// ####################                          Only stuff below this is currently used!
// ####################
// ####################
// ####################
// ####################
...
bool TstarTstar_tgtg_TopTag_Reconstruction::process(uhh2::Event& event){
  ...
  for(uint i = 0; i < event.topjets->size(); i++){
	...
	for(uint j = 0; j < event.topjets->size(); j++){
	  ...
	  else if(gluonCands.size() == 2) continue; // only take leading two gluon candidates. shortens calculation time.
	...
	ttbar_hyp.set_tophad_topjet_ptr(&tj);
