...
TstarTstarElectronIDHists::TstarTstarElectronIDHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  book<TH1F>("count_beforettbargen", "N ele", 1, 0, 1);
  book<TH1F>("count_beforettbargen_after", "N ele", 1, 0, 1);

  book<TH1F>("count_ttbargen", "N ele", 1, 0, 1);
  book<TH1F>("count_ttbargen_after", "N ele", 1, 0, 1);
...
void TstarTstarElectronIDHists::fill(const Event & event){
  ...
  if(event.electrons->size()==1) {
    ...
    if(event.electrons->at(0).pt()>120) pass_2D = pass_twodcut;
    else pass_2D = true;
  }
  ...
  if(event.electrons->size() == 1) {
    ...
    for (const auto & jet : *event.jets) {
      double dR_tmp = deltaR(jet, event.electrons->at(0));
      if(dR_tmp < dR) dR = dR_tmp;
    }
    if(event.electrons->at(0).pt() > 120) dR_cut = dR_cut_high;
    else dR_cut = dR_cut_low;
    if((dR > dR_cut) && (!do2Dcut || (do2Dcut && pass_2D)) && (!doAddMETcut || (doAddMETcut && (event.met->pt() > 80)))) hist("count_beforettbargen_after")->Fill(0.5, weight);
  }
  ...
  if(event.electrons->size() == 1) {
    ...
    if ((dR > dR_cut) && (!do2Dcut || (do2Dcut && pass_2D)) && (!doAddMETcut || (doAddMETcut && (event.met->pt() > 80)))) {
      hist("count_ttbargen_after")->Fill(0.5, weight); // counting selection efficiency
      if(deltaR(ttbargen.ChargedLepton(), event.electrons->at(0)) < 0.2) hist("sel_matched_to_gen")->Fill(1., weight);
